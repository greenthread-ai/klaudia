...
async function showSetupScreens(A, q, K, Y, z) {
  ...
  if (z && !k1().hasCompletedClaudeInChromeOnboarding) {
...
function wCz() {
  (J8((q) => ({ ...q, numStartups: (q.numStartups ?? 0) + 1 })), $Cz());
  let A = O5(Q_6() ?? KW());
  kR1(y1(), fX(A, iH()));
}
function _Cz() {
...
`);
        ...
        let aY = {
          ...
          R5 = {
            settings: U7(),
            tasks: {},
            verbose: g ?? k1().verbose ?? !1,
            mainLoopModel: GA,
            mainLoopModelForSession: null,
            isBriefOnly: !1,
            expandedView: k1().showSpinnerTree
              ? "teammates"
              : k1().showExpandedTodos
                ? "tasks"
                : "none",
